...
async function testHODNotificationRouting() {
  console.log('\n🧪 Testing HOD Notification Routing');
  console.log('===================================\n');

  try {
    // Find the most recent notification for HOD about corrective actions
    const recentNotification = await prisma.notification.findFirst({
      where: {
        type: 'CORRECTIVE_ACTION_COMMITTED',
        title: {
          contains: 'Correction Requirement Committed'
        }
      },
      orderBy: {
        createdAt: 'desc'
    ...
    if (!recentNotification) {
      console.log('❌ No recent corrective action notifications found');
      console.log('   Try committing a correction requirement first');
      return;
    }
    ...
    console.log(`   Target User: ${recentNotification.targetUser.name} (${recentNotification.targetUser.email})`);
    ...
    if (nonConformity) {
      ...
      if (relatedCA) {
        ...
        console.log(`   ✅ HOD will see this corrective action when they click the notification`);
      }
      ...
      console.log(`   ❌ HOD will get "non-conformity not found" error`);
    } else {
    ...
    console.log(`\n🔄 Workflow Simulation:`);
    console.log(`   1. ✅ Team Leader committed correction requirement`);
    console.log(`   2. ✅ System created notification for HOD: ${recentNotification.targetUser.name}`);
    console.log(`   3. ✅ Notification link: ${recentNotification.link}`);
    console.log(`   4. ${nonConformity ? '✅' : '❌'} HOD clicks notification`);
    ...
    console.log(`   7. ${nonConformity ? '✅' : '❌'} HOD can respond to correction requirement`);

    if (nonConformity) {
      console.log(`\n🎉 SUCCESS: HOD Notification Routing is Working Correctly!`);
      console.log(`   - Notification uses nonConformityId parameter: ${routeParam}`);
      console.log(`   - Routes to existing frontend page structure`);
      console.log(`   - HOD can access and respond to corrective actions`);
      console.log(`   - End-to-end workflow is functional`);
    } else {
      console.log(`\n❌ FAILED: HOD Notification Routing Needs Fix`);
...
// Run the test
testHODNotificationRouting().catch(console.error);
